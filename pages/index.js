...
export default function Home() {
  ...
  const buyNFT = async (nft) => {
    ...
    const transation = await contract.createMarketSale(nftaddress, nft.tokenId, {
      value: price
    })
  ...
  if(loading === "loaded" && !nfts.length)
    return(
      <h1 className="px-20 py-7 text-4x1 font-bold">No NFTs</h1>
    )
            ...
            nfts.map((nft, index) => (
              <div key={index} className="border shadow rounded-x1 overflow-hidden">
                ...
                <div className="p-4 bg-black">
                  ...
                  <button className="w-full bg-purple-500 text-white font-bold py-3 px-12 rounded" onClick={() => buyNFTs(nft)}>
                    Buy
                  </button>
                </div>
              </div>
